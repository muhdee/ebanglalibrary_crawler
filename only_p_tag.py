...
def extract_wp_block_paragraphs(soup):
    """Layout A: multiple <p class="wp-block-paragraph"> tags."""
    paragraphs = soup.find_all("p", class_="wp-block-paragraph")
    return [p.get_text(strip=True) for p in paragraphs]


def extract_ld_tab_content_paragraphs(soup):
    """Layouts B & C: paragraphs live inside
    <div class="ld-tab-content ld-visible entry-content">.

    Handles both a single <p> and multiple <p> tags inside the div. Each
    <p> tag is itself split on any internal <br> tags, since some pages mix
    both patterns (multiple <p> tags where individual <p> tags may still
    contain <br>-separated paragraphs).
    """
    container = soup.select_one("div.ld-tab-content.ld-visible.entry-content")
    if not container:
        return []
    ...
    paragraphs = []
    for p_tag in p_tags:
        # Turn each <br> into a newline marker so it splits into its own
        # paragraph, even when this <p> is one of several in the div.
...
def fetch_page_paragraphs(url):
    """Fetches a page and returns its list of paragraph texts.

    Tries Layout A first (class="wp-block-paragraph" paragraphs). If that
    finds nothing, falls back to the ld-tab-content div, which may contain
    either multiple plain <p> tags (Layout C) or a single <p> with
    <br>-separated text (Layout B).
    """
    ...
    paragraphs = extract_wp_block_paragraphs(soup)
    if paragraphs:
        return paragraphs

    paragraphs = extract_ld_tab_content_paragraphs(soup)
    if paragraphs:
        return paragraphs
